	...
	then
		...
		if [ -z "$2" ]
			then
				...
				g++ -std=c++17 $1 -o $a `pkg-config --cflags --libs opencv4 libvlc` -lpthread  -lmysqlcppconn -lstdc++fs
			else
				# g++ -std=c++0x $1 -o $2 `pkg-config --cflags --libs opencv` -lpthread  -lmysqlcppconn
				g++ -std=c++17 $1 -o $2 `pkg-config --cflags --libs opencv4 libvlc` -lpthread  -lmysqlcppconn -lstdc++fs 
		fi 	
